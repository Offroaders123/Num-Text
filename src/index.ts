...
class NumTextElement extends HTMLElement {
  ...
  constructor(){
    ...
    this.themes = {
      ...
      has: (name: string) => (name in this.themes.entries),
      enable: (name: string) => {
  ...
  connectedCallback(){
    ...
    this.gutter.addEventListener("dblclick",event => {
      var indexes = this.getLineIndexes(), line = Array.from(this.gutter.children).indexOf(event.target as HTMLElement);
  ...
  refreshLineNumbers(){
    this.refreshSyntaxOverlay();
    var previousCount = Number(this.getAttribute("line-count") || 0), count = (this.editor.value.match(/\n/g) || []).length + 1, difference = count - previousCount;
